...
(function(win) {
	...
	theme = theme || getCookie('_theme_') || 'idea';
	skin = getCookie('_' + theme + '_skin_') || (THEME_CONFIG[theme] && THEME_CONFIG[theme]['defaultSkin']) || 'violet';
	...
	function setCookie(name, value, expires, domain) {
		...
		document.cookie = name + '=' + escape(value) + (expires === null ? '' : '; expires=' + largeExpDate.toGMTString()) + ';path=' + (_rootPath ? _rootPath : '/;')
	...
	var SrcBoot = {
		...
		getThemeSkins : function(themeName, withDisabled) {
			...
			return withDisabled ? skins : skins.filter(function(item) {
		...
		genderLink : function(data) {
			return this._linkTpl.replace(/\${id}/, data.id ? 'id="' + data.id + '"' : '').replace(/\${alternate}/, data.alternate).replace(/\${disabled}/, data.alternate ? "disabled" : "").replace(
					/\${title}/, data.title).replace(/\${type}/, data.type).replace(/\${name}/, data.name).replace(/\${href}/, data.href);
		},
		/**
		 * 输出皮肤资源
		 * 
		...
		outputCustomSkin : function(skinPath) {
			var skins = this.getThemeSkins(theme), cskin,
				delayArr = [],
				isCurr = false,
				linkStr = '',
				i = 0,
			...
			for (; i < l; i++) {
				...
				alternate = isCurr ? '' : ' alternate';
				// linkStr = '<link rel="stylesheet' + (isCurr ? '' : ' alternate') + '" name="' + name + '" data-name="' + cskin.name + '" title="' + (cskin.cname || cskin.name) + '" href="' +
				// this.handleResPath(skinPath + cskin.name + '/skin.css') + '"/>';
		...
		/*
		 * 是否调试环境 当debug为false，则全部使用压缩代码，适合生产环境 当debug为true时， 1. develop为false，则使用有除miniui外的未压缩代码，适合项目开发环境 2. develop为true， 则全部使用未压缩代码，适合框架开发环境
		 */
		debug : false,
	...
	var isIndexActive = localStorage.getItem('isIndexActive'),
		curThemeSkin = localStorage.getItem('curSkin'),
		thisSkin = (isIndexActive === 'true') ? 'violet' : ((curThemeSkin) ? curThemeSkin : skin);

	// 检查皮肤 确定皮肤是有效值 避免无效 cookie 导致页面完全无法显示
	!(function fixSkin() {
		var list = SrcBoot.getThemeSkins(theme);
		if (!list.some(function(item) {
			return item.name === thisSkin;
		})) {
	...
	SrcBoot.output(arr);
	// 输出控件皮肤 主界面皮肤
	SrcBoot.outputSkin(isThemePage, theme, SrcBoot.getThemeSkins(theme, true), thisSkin);
